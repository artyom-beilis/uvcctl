...
public:
    ...
    Derotator(float lon_d,float lat_d) : lon_d_(lon_d), lat_d_(lat_d)
    {
    }
    ...
    float getAngleDeg(float RAd,float DEd,double time0_s,double time1_s)
    {
...
int main()
{
    float lon = 31.89274203158037f, lat= 34.80648087724496f;

    Derotator dr(lat,lon);
    
    time_t from = 1669551938 + 3600*(10);
    time_t next = 1669551938 + 3600*(11);
    float RA=10.6848f,DE=41.2691;
    float alt = std::asin(dr.rayFromPos(RA,DE,from).z) / 3.14159 * 180;
    char buf[256];
    strftime(buf,sizeof(buf)," %Y-%m-%d %H:%M:%s",localtime(&from));
    std::cout << "Alt=" << alt << buf << std::endl;

    float angle = dr.getAngleDeg(RA,DE,from,next);
